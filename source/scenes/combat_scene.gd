...
@onready var t_player := preload(AssetUtility.entity_path + "player.tscn")
@onready var t_enemy := preload(AssetUtility.entity_path + "enemy.tscn")
...
## 对于Controller层的引用
var controller: CombatSceneLogic # 在切换场景的时候注入
